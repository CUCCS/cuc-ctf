## misc

平台：buuoj


- 二维码  

  扫描二维码没有 flag，使用 binwalk 可以提取出一个压缩包，内部有个 txt 文件，但是没有内容。

  解压压缩包需要密码。

  ![](imgs/zip_content.png)

  以为是伪加密，修改1400 后面的 4位数字， 0900 为 0000 ，修改两处，任然没有办法解开。

  尝试进行密码的爆破，txt 文件名叫 4number，所以用 4 位数字爆破

  - fcrackzip

    ```
    sudo apt install fcrackzip
    ```

    ```bash
    # －b 指定模式为暴破，-c1指定密码类型为纯数字，其它类型可以rtfm,-u这个参数非常重要不然不显示破解出来的密码,-l 5-6可以指定长度
    cuc@cuc-VirtualBox:/mnt/sharefolder/ctf/_QR_code.png.extracted$ fcrackzip -b -c1 -l 4 -u ./1D7.zip 
    PASSWORD FOUND!!!!: pw == 7639
    ```

  - windows 平台上的 advanced-archive-password-recovery-4-54-110.msi、和 AZPR_4.0 都可以进行爆破。

- N种解决方法

  解开压缩包是个 exe 文件，查看二进制，内部后 jpeg 字样，文件内容是data协议，直接输入到浏览器就能解析出一个二维码。

  - data 协议

    ```bash
    data://text/plain;base64,
    data:资源类型;编码,内容
    data:[<mime type>][;charset=<charset>][;base64],<encoded data>
    ```

  ```
  data:image/jpg;base64,
  ```

- [SUCTF2018]single dog

  binwalk 提取出一个压缩包，内部一个 txt 文件，是颜文字混淆的 js 代码。 浏览器 F12 在console 内输入，弹出一个消息框。找一个解密颜文字的[在线网站](http://www.atoolbox.net/Tool.php?Id=703)，flag 以变量的形势存储在 js 代码中。

